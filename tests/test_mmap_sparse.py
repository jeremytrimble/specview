...
def test_mmap_sparse_file(tmpdir):
    """
    ...
    temp_file = Path(tmpdir) / "sparse_test.dat"
    ...
    with open(temp_file, "wb") as f:
        ...
        data1 = b"FIRST_REGION_DATA_CONTENT_123"
        f.write(data1)
        f.flush()
    
    # Step 2: Memory-map the non-sparse region
    with open(temp_file, "r+b") as f:
        # Map the region where we wrote data1 (starting at offset 4096)
        mm1 = mmap.mmap(f.fileno(), len(data1), offset=4096)
        
        # Verify the first mapping contains the expected data
        assert mm1[:] == data1, "First mapping should contain the data we wrote"
        ...
        f.seek(0)
        data2 = b"SPARSE_REGION_NOW_FILLED_456"
        f.write(data2)
        f.flush()
        
        # Step 4: Create a second mapping for the newly written (formerly sparse) region
        mm2 = mmap.mmap(f.fileno(), len(data2), offset=0)
        
        # Assert that the first mapping is still usable and returns expected contents
        assert mm1[:] == data1, "First mapping should still contain original data"
        
        # Assert that the second mapping returns the right contents
        assert mm2[:] == data2, "Second mapping should contain the newly written data"
    ...
    with open(temp_file, "rb") as f:
        f.seek(0)
        assert f.read(len(data2)) == data2, "Sparse region data should persist"
        
        f.seek(4096)
        assert f.read(len(data1)) == data1, "Original non-sparse region data should persist"
